...
function Menus ({children}) {
  ...
  const [position, setPosicion] = useState(null);
  const close = () => setOpenId('');
  const open = setOpenId;

  return (
    <MenusContext.Provider value={{openId, close, open, position, setPosicion}}>
      {children}
...
function Toggle({id}){
  const {open, close, openId, setPosicion} = useContext(MenusContext);
